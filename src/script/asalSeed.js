...
/**
 * Data Provinsi & Kabupaten/Kota se-Indonesia (38 Provinsi).
 * Nama kota diawali "Kota " agar mudah dibedakan dari kabupaten.
 * Sumber acuan umum: Kemendagri/BPS. Karena pemekaran wilayah kadang
 * berubah (terutama di Papua), disarankan verifikasi ulang sebelum
 * dipakai di data produksi.
 */
const data = {
    Aceh: [
    ...
    "Papua Barat Daya": ["Kabupaten Kota Sorong", "Kabupaten Maybrat", "Kabupaten Raja Ampat", "Kabupaten Sorong", "Kabupaten Sorong Selatan", "Kabupaten Tambrauw", "Kota Sorong"],
    "Papua Tengah": [
        "Kabupaten Deiyai",
        "Kabupaten Dogiyai",
...
async function main() {
    ...
    for (const [namaProvinsi, kabupatenList] of Object.entries(data)) {
        const provinsi = await prisma.provinsi.upsert({
            where: { namaProvinsi: namaProvinsi },
            update: {},
            create: { namaProvinsi: namaProvinsi },
        });
        totalProvinsi++;

        for (const namaKabupaten of kabupatenList) {
            // namaKabupaten unik secara global (bukan hanya per-provinsi),
            // jadi pakai upsert langsung berdasarkan nama.
            const result = await prisma.kabupaten.upsert({
                where: { namaKabupaten: namaKabupaten },
                update: { provinsiId: provinsi.id },
                create: {
            ...
            if (result) totalKabupaten++;
        }

        console.log(`✔ ${namaProvinsi} (${kabupatenList.length} kabupaten/kota)`);
    }

    console.log(`\nSelesai. ${totalProvinsi} provinsi diproses, ${totalKabupaten} kabupaten/kota diproses.`);
}
